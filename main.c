...
int** readFromCSV(char* fileName,int* num )
{
    char buffer[1024] ;
   char *record,*line;
   int i=0,j=0,n=0;
   ...
   n=i;
   int **users;
   users = malloc(sizeof(int*)*n);
   for(int i=0;i<n;i++)
   {
    users[i]=malloc(sizeof(int)*n);
    users[i][0]=mat[i][0];
    users[i][1]=mat[i][1];
    users[i][2]=mat[i][2];
    users[i][3]=mat[i][3];
...
int findIDincsv(uint16 id, int** userlist, int n)
{
	for(int i=0;i<n;i++)
	{
		if(userlist[i][0]==id)
...
void updateAccountBalance(char* fileName, int** userlist, int n, int user, uint32 cashDetails)
{
	userlist[user][3]-=cashDetails;
   ...
   fprintf(fstream, "%s,%s,%s,%s\n","User ID (decimal)","PIN Hash (decimal)","Admin","Balance (decimal)" );
   for(int i=0;i<n;i++)
	{
		fprintf(fstream,"%i,%i,%i,%i\n",userlist[i][0],userlist[i][1],userlist[i][2],userlist[i][3]);
	}
   fclose(fstream);
}

void encrypt (uint8* v) {
    uint32_t v0=(v[3]<<24)|(v[2]<<16)|(v[1]<<8)|v[0], v1=(v[7]<<24)|(v[6]<<16)|(v[5]<<8)|v[4], sum=0, i;          /* set up */
    uint32_t delta=0x9e3779b9;                     /* a key schedule constant */
    uint32_t k0=0x00000000, k1=0x00000000, k2=0x00000000, k3=0x00000000;   /* cache key */
    for (i=0; i < 32; i++) {                       /* basic cycle start */
        sum += delta;
        v0 += ((v1<<4) + k0) ^ (v1 + sum) ^ ((v1>>5) + k1);
        v1 += ((v0<<4) + k2) ^ (v0 + sum) ^ ((v0>>5) + k3);
    }                                              /* end cycle */
    v[3]=v0>>24; v[2]=(v0>>16)&255; v[1]=(v0>>8)&255; v[0]=v0&255;
    v[7]=v1>>24; v[6]=(v1>>16)&255; v[5]=(v1>>8)&255; v[4]=v1&255;
}

void decrypt (uint8* v) {
    uint32_t v0=(v[3]<<24)|(v[2]<<16)|(v[1]<<8)|v[0], v1=(v[7]<<24)|(v[6]<<16)|(v[5]<<8)|v[4], sum=0xC6EF3720, i;  /* set up */
    uint32_t delta=0x9e3779b9;                     /* a key schedule constant */
    uint32_t k0=0x00000000, k1=0x00000000, k2=0x00000000, k3=0x00000000;   /* cache key */
    for (i=0; i<32; i++) {                         /* basic cycle start */
        v1 -= ((v0<<4) + k2) ^ (v0 + sum) ^ ((v0>>5) + k3);
...
int main(int argc, char *argv[]) {
	...
	//struct arg_lit *itrOpt  = arg_lit0("y", "iterations", "                    displaying, modifying channels ");
	...
	void *argTable[] = {
		ivpOpt, vpOpt, //fwOpt, portOpt, queryOpt, progOpt, conOpt, actOpt, shellOpt, itrOpt, benOpt, rstOpt, dumpOpt, helpOpt, eepromOpt, backupOpt, 
		endOpt
	};
	...
	//if ( itrOpt->count ) {
		...
		if ( isCommCapable ) {
			...
			if ( isRunning ) {
				...
				uint16 id, pin;
				uint32 cashDetails;
				//size_t oldLength,s=1;
				//data = malloc(s);
				int n_users,user;
				int **userlist=readFromCSV("SampleBackEndDatabase.csv",&n_users);
				while(1)
				{
					flSleep(1000);
					chan=0;
					// Getting to the end of the buffer.
					//oldLength = dataFromFPGA.length;	
					//bStatus = bufAppendConst(&dataFromFPGA, 0x00, length, &error);
					//CHECK_STATUS(bStatus, FLP_LIBERR, cleanup);
					// Reading from the channel i as output "out".
					fStatus = flReadChannel(handle, (uint8)chan, length, &data, &error);// Changes the channel, f2hData_next, reads f2hData by a lag.
					CHECK_STATUS(fStatus, FLP_LIBERR, cleanup);
					flSleep(1);
					fStatus = flReadChannel(handle, (uint8)chan, length, &data, &error);// Reads from f2hData without a lag.
					CHECK_STATUS(fStatus, FLP_LIBERR, cleanup);
					printf("%02X\n", data);
					r1=data;
					if(r1==0x01 || r1==0x02)
					{
						...
						if(r1==r2)
						{
							...
							if(r1==r3)
							{
								...
								for(int i=0;i<8;i++)
								{
									fStatus = flReadChannel(handle, (uint8)(i+1), length, &data, &error);
									CHECK_STATUS(fStatus, FLP_LIBERR, cleanup);
									flSleep(1);
									fStatus = flReadChannel(handle, (uint8)(i+1), length, &data, &error);
									CHECK_STATUS(fStatus, FLP_LIBERR, cleanup);
								...
								pin=(input[2]<<8)|input[3];
								cashDetails=(input[4]*2000)+(input[5]*1000)+(input[6]*500)+(input[7]*100);
								pin=(uint16)(pin<<11)|(uint16)(pin>>5);
								user=findIDincsv(id,userlist,n_users);
								if(user>=0 && userlist[user][1]==pin)
								{
									if(userlist[user][2])
									{
										...
										{
											data=input[i];
											fStatus = flWriteChannel(handle, (uint8)(10+i), length, &data, &error); 
											CHECK_STATUS(fStatus, FLP_LIBERR, cleanup);
											flSleep(1); // No need to make write request again.
										}
										data=0x03;
										fStatus = flWriteChannel(handle, (uint8)9, length, &data, &error);
										CHECK_STATUS(fStatus, FLP_LIBERR, cleanup);
										flSleep(1);
										fStatus = flReadChannel(handle, (uint8)(0), length, &data, &error);
										CHECK_STATUS(fStatus, FLP_LIBERR, cleanup);
										flSleep(1);
										fStatus = flReadChannel(handle, (uint8)(0), length, &data, &error);
										CHECK_STATUS(fStatus, FLP_LIBERR, cleanup);
									...
									else
									{
										...
										if(userlist[user][3]>=cashDetails)
										{
											input[0]=input[4];
											input[1]=input[5];
											input[2]=input[6];
										...
										printf("%02X\n", data);
											...
											if(r1==0x01)
											{
												updateAccountBalance("SampleBackEndDatabase.csv",userlist,n_users,user,cashDetails);
											}
										}
										else
										{
								...
								else
								{
									...
									data=0x04;
									// Writing out to the channel (i+1)mod128.
									fStatus = flWriteChannel(handle, (uint8)9, length, &data, &error);
									CHECK_STATUS(fStatus, FLP_LIBERR, cleanup);
									flSleep(1); // No need to make write request again.
									fStatus = flReadChannel(handle, (uint8)(0), length, &data, &error);
				...
				/*for(i=0;i<128;i++){
					...
					fStatus = flReadChannel(handle, (uint8)chan, length, dataFromFPGA.data + oldLength, &error);
					CHECK_STATUS(fStatus, FLP_LIBERR, cleanup);
					printf("%02X\n", *(dataFromFPGA.data + oldLength));
					// Adding i to out.
					*data=(*(dataFromFPGA.data + oldLength)+i);
					// Writing out to the channel (i+1)mod128.
					fStatus = flWriteChannel(handle, (uint8)(chan+1 &127), s, data, &error);
					CHECK_STATUS(fStatus, FLP_LIBERR, cleanup);
